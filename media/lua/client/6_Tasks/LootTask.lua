...
function LootCategoryTask:ForceFinish()
	...
	if(MyFindAndReturnCategory(self.parent:getBag(),"Weapon") ~= nil) and (self.parent:Get():getPrimaryHandItem() == nil) then
		self.parent:Get():setPrimaryHandItem(MyFindAndReturnCategory(self.parent:getBag(),"Weapon"))
	end
		...
		elseif(not self.parent:hasRoomInBag()) then
			...
			elseif(instanceof(self.Container,"ItemContainer")) then
				...
				if(distance > 2.1) or (self.parent.player:getZ() ~= self.Floor) then 
					...
					if(self.Container:getSourceGrid()) and (self.parent:getWalkToAttempt(self.Container:getSourceGrid()) > 8) then self.Container = nil end -- cant get to it 9blocked)
				
				else
					self.parent:DebugSay("else")
					local item = MyFindAndReturnCategory(self.Container,self.Category)
					if(item ~= nil) then
